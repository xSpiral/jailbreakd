...
uint32_t
kernel_read_32(uint64_t address, void *data, size_t size) {
    uint32_t *value = data;
    bool ok = kernel_read(address, &value, size);
    if (!ok) {
        return -1;
    }
    return *value;
...
//----------pwn20 Kernel Read/Write functions------

#define KERNEL_SEARCH_ADDRESS 0xfffffff007004000

//uint64_t
//returnKernelBase(uint64_t kernel_slide) {
//    return kernel_slide + KERNEL_SEARCH_ADDRESS;
//}
//
//
...
//        }
//        
//        rv = mach_vm_read_overwrite(kernel_task_port,
//                                    where + offset,
//                                    chunk,
...
//            chunk = size - offset;
//        }
//        rv = mach_vm_write(kernel_task_port,
//                           where + offset,
